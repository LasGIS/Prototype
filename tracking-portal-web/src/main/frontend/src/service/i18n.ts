...
const resources = {
  en: {
    translation: {
      ...
      "unlimited.false.1": "\\ To get access to unlimited tracking, you need to sign a contract with Russian Post and contact your personal manager at Russian Post with a request to remove the restrictions.",
      ...
      "incomplete-profile.explanation-prefix": "You should specify your name, second name and email to use Tracking API. Please fill this properties in your",
      ...
      "LandingMessages.leftColumn": "<header>Single access</header><section>One shipment per request.</section><section>Up to 100 requests per day.</section><section>Unlimited access is granted</section><section>to clients <a href=\"/support\">as per contract</a>.</section>",
      "LandingMessages.middleColumn": "<header>Batch access</header><section>Up to 3000 shipments per request.</section><section>Batch access is granted</section><section>only to clients <a href=\"/support\">as per contract</a>.</section>",
      "LandingMessages.rightColumn": "<header>Statistics</header><section>Number of requests per day,</section><section>overlimit control,</section><section>email notifications.</section>",
      ...
      "dictionary.operationCodes.note": "Note: for some operations, which require the attribute, API of shipment tracking may return attribute value of 0. The Client should interprete such attribute value as the absence of information on the attribute. This rule has only one exception: operation 8 (processing), which has attribute 0 (Sorting) as normal.",
      ...
      "dictionary.postmark.note": "Several mail marks may be assigned to one shipment. In the table below code value of each succeedent mark is two times greater than the code value of the preceding one, except for the value of the code «No code». Thus, every sum of code values of the marks assigned to any shipment is a unique number, which is used for generating data about the shipment.",
      ...
      "dictionary.sendCtg.note2": "Code «Post operator» is used to identify sender category of international shipments received from private operators at the office of exchange in Germany and sent to Russian Federation.",
  ...
  ru: {
    translation: {
      ...
      "unlimited.false.1": "Для получения безлимитного доступа необходимо оформить договор с Почтой России и обратиться к своему персональному менеджеру в Почте России с запросом на снятие ограничений.",
      ...
      "incomplete-profile.explanation-prefix": "Для работы с API отслеживания необходимо указать фамилию, имя и email. Для этого перейдите, пожалуйста, в свой",
      ...
      "LandingMessages.leftColumn": "<header>Единичный доступ</header><section>Одно отправление в запросе.</section><section>Лимит до 100 запросов в сутки, <a href=\"/support\"> с договором </a> — без ограничений</section>",
      "LandingMessages.middleColumn": "<header>Пакетный доступ</header><section>До 3000 отправлений в запросе.</section><section>Доступен только клиентам <a href=\"/support\">с договором </a></section>",
      "LandingMessages.rightColumn": "<header>Статистика</header><section>Количество запросов в день,</section><section>контроль превышения лимита,</section><section>оповещения на электронную почту</section>",
      ...
      "dictionary.sendCtg.note1": "Код «Международный оператор» используется для идентификации категории отправителя входящих или транзитных международных почтовых отправлений.",
      "dictionary.sendCtg.note2": "Код «Почтовый оператор» используется для идентификации категории отправителя международных почтовых отправлений, принятых в месте обмена за границей в Германии от частных операторов и направляемых в РФ.",
