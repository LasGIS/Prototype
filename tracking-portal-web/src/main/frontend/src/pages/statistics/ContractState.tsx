...
class ContractState extends Component<Props> {
  ...
  onScroll(): any {
    ...
    const current: HTMLDivElement | null = this.contractStateRef.current;
    if (current) {
  ...
  onClose(ev: MouseEvent, action: () => any) {
    ev.stopPropagation();
    action();
  }

  onClick(ev: MouseEvent, action?: () => any) {
    if (action) {
      action();
    }
  }
  ...
  render() {
    const { t } = this.props;
    const stateDesc = consistStates.find((st: Consist) => st.unlimited === this.props.unlimited);
    if (!stateDesc) {
      return null;
    }
      ...
      <div>
        <div className="contract-state contract-state--bottom">
          <div className="contract-state__paper">
            ...
            <div className={cn("contract-state__text", { "contract-state__text--link": action })}
                 onClick={(event: MouseEvent) => this.onClick(event, action)}>
              {info.map((textKey, key) => {
                return <div key={key}>{t(textKey)}</div>;
              })}
              {close && <div className="contract-state__close" onClick={(event: MouseEvent) => this.onClose(event, close)}/>}
            </div>
          </div>
        ...
        <div className="contract-state contract-state--hide" ref={this.contractStateRef}>
          <div className="contract-state__paper">
            <div className={iconClass}/>
            <div className={cn("contract-state__text", { " contract-state__text--link--float": action })}
                 onClick={(event: MouseEvent) => this.onClick(event, action)}>
              {
                info.map((textKey, key) => {
                  return key < 2 ? <div key={key}>{t(textKey)}</div> : null;
                })
              }
            </div>
          </div>
          <div className="contract-state__band contract-state__band--float">
            {close ? <div className="contract-state__close" onClick={(event: MouseEvent) => this.onClose(event, close)}/> : null}
          </div>
        </div>
      </div>
    );
