...
export const getMonths = (t: TFunction): string[] => {
  return [
    t('stat.month.jan'), t('stat.month.feb'),
    t('stat.month.mar'), t('stat.month.apr'),
    t('stat.month.may'), t('stat.month.jun'),
    t('stat.month.jul'), t('stat.month.aug'),
    t('stat.month.sep'), t('stat.month.oct'),
    t('stat.month.nov'), t('stat.month.dec')
  ];
}

export const getFormatMonths = (t: TFunction): string[] => {
  return [
    t('stat.month.jan.full'), t('stat.month.feb.full'),
    t('stat.month.mar.full'), t('stat.month.apr.full'),
    t('stat.month.may.full'), t('stat.month.jun.full'),
    t('stat.month.jul.full'), t('stat.month.aug.full'),
    t('stat.month.sep.full'), t('stat.month.oct.full'),
    t('stat.month.nov.full'), t('stat.month.dec.full')
  ];
}
...
export const fillArray = (
  fromTime: StatisticsDataItem,
  toTime: StatisticsDataItem
): StatisticsDataChart[] => {
  ...
  while (currentDate.getTime() < toDate.getTime()) {
    ...
    if (fromTime.hasOwnProperty("r")) {
      dataChart.response = 0;
    }
    if (fromTime.hasOwnProperty("limit")) {
...
export const getStartZoomSelection = (data: StatisticsDataChart[]): ZoomIndexes => {
  ...
  return length > 0 ? {
    start: length > 7 ? length - 7 : 0, end: length - 1
  } : {
    start: 0, end: 0
  };
}

export const checkStartEndIndex = (
  data: StatisticsDataChart[], start?: number, end?: number
): ZoomIndexes => {
  const length = data.length;
